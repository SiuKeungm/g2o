...
void BuildProblem(const BALProblem* bal_problem, g2o::SparseOptimizer* optimizer, const BundleParams& params)
{
    ...
    // Set camera vertex with initial value in the dataset.
     const double* raw_cameras = bal_problem->cameras();//raw_cameras指向的数据为常数
    for(int i = 0; i < num_cameras; ++i)
    {

        raw_cameras+camera_block_size*i;

        CameraSO3 cam(Sophus::SO3(raw_cameras[0],raw_cameras[1],raw_cameras[2]),
                      Eigen::Vector3d(raw_cameras[3],raw_cameras[4],raw_cameras[5]),
                      raw_cameras[6],raw_cameras[7],raw_cameras[8]);
        VertexCameraBAL* pCamera = new VertexCameraBAL();
        pCamera->setEstimate(cam);   // initial value for the camera i..
...
void WriteToBALProblem(BALProblem* bal_problem, g2o::SparseOptimizer* optimizer)//数据改变时用来更新定点的原始估计值
{
    const int num_points = bal_problem->num_points();
    const int num_cameras = bal_problem->num_cameras();
    const int camera_block_size = bal_problem->camera_block_size();
    ...
    for(int i = 0; i < num_cameras; ++i)
    {
        ...
        Eigen::Vector3d so3_estimate = pCamera->estimate().so3_.log();
        Eigen::Matrix<double,9,1> NewCameraVec;//返回的是向量的引用
        NewCameraVec.block(0,0,3,1) = so3_estimate;
        NewCameraVec.block(3,0,3,1) = pCamera->estimate().t_;
        NewCameraVec(6) = pCamera->estimate().f_;
        NewCameraVec(7) = pCamera->estimate().k1_;
        NewCameraVec(8) = pCamera->estimate().k2_;
        memcpy(raw_cameras + i * camera_block_size, NewCameraVec.data(), sizeof(double) * camera_block_size);//内存拷贝函数(dest,src,,拷贝大小）
    }
...
void SetLinearSolver(std::shared_ptr<BalBlockSolver>& solver_ptr, const BundleParams& params)
{
    ...
    solver_ptr = std::make_shared<BalBlockSolver>(linearSolver);//make_shared 在动态内存中分配一个对象并初始化它，括号里为初始化BlockSolve的构造函数
